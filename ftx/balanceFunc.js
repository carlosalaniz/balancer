    ...
    try {
        ...
        if (absDelta > 0) {            
            ...
            if (absDelta >= min_delta && absDelta <= max_delta) {
                await doTransaction(ftxClient, delta, market, transactionObject);
            } else {
...
const doTransaction = async function (ftxClient, amount, market, transactionObject) {
    ...
    if (amount > 0) {
        await ftxClient.goLongBy(absAmount, market);
        transactionObject.side = "BUY";
    } else {
        await ftxClient.goShortBy(absAmount, market);
        transactionObject.side = "SELL";
    }

    // after the transaction is performed, update the transaction object.
    transactionObject.status = "COMPLETE";
    transactionObject.position_after_transaction = (await ftxClient.getPosition(market)).size;
}
